...
FX_RESON* new_reson()
{
	...
	reson->coeffs.a0 = reson->coeffs.a1 = reson->coeffs.a2 
		= reson->coeffs.b1 = reson->coeffs.b2 = reson->ynm1 = reson->ynm2 = 0.0;	
...
/********** Second-order classic RESON filter with two poles : Dodge & Jerse 2nd ed. p 211*/
...
int allpole_init(FX_RESON* flt, unsigned long srate,double cf,double bw)
{
	...
	flt->coeffs.a0 = (1.0 - b2) * sqrt(1.0 - (b1 * b1) / (4.0 * b2));
...
float allpole_utick(FX_RESON* flt, float input, double cf,double bw)
{
	...
	output = flt->coeffs.a0 * input - flt->coeffs.b1 * flt->ynm1 - flt->coeffs.b2 * flt->ynm2;
	flt->ynm2 = flt->ynm1;
	flt->ynm1 = output;
	return (float) output;	
}
...
void allpole_set_bw(FX_RESON* flt, double bw)
{
	...
	if(bw == flt->bw)
		return;
    b2 = exp(-flt->srfac * bw);
    b1 = (-4.0 * b2) / (1.0 + b2) * cos(flt->srfac * flt->cf);
	flt->coeffs.a0 = (1.0 - b2) * sqrt(1.0 - (b1 * b1) / (4.0 * b2));
    flt->coeffs.b1 = b1;
	flt->coeffs.b2 = b2;
	flt->bw = bw;
