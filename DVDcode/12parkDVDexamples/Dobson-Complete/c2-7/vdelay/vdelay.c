...
/* STAGE 2 */	
	...
	if(argc < ARG_NARGS){
		printf("insufficient arguments.\n"
			...
			"  freq    :  modulation frequency (Hz). 0.0 < freq < Nyquist.\n"
...
/* STAGE 3 */																							
	...
	if(modfreq >= inprops.srate / 2){
		printf("Error: freq value too high: must be < %ld\n",inprops.srate);
		error++;
		goto exit;
	}
	...
	*/
	...
	if(osc == NULL){
		puts("VDELAY: unable to create modulation oscillator\n");
		error++;
		goto exit;
	}
	/* create irand osc */
	...
	while ((framesread = psf_sndReadFloatFrames(infile,inframe,nframes)) > 0){
		...
		if(do_randmod){
			...
			for(i=0;i < framesread;i++){
				...
				delay_delta *= vdelaytime;
				tickval =  fxdelay_tick(delay, vdelaytime + delay_delta, feedback, input);
				inframe[i] = (float)( dry * input + wet * /*fxdelay_tick(delay, vdelaytime + delay_delta, feedback, input)*/tickval);				
			}
		}
