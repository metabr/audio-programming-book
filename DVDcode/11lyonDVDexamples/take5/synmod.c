...
main(int argc, char **argv)
{
	...
	ringmods = (RINGMOD *) malloc(MAXMODS * sizeof(RINGMOD));
	reverbs = (REVERB *) malloc(MAXMODS * sizeof(REVERB));
	...
	while( scanf("%s", modname ) != EOF){
		...
		else if(! strcmp(modname, "RINGMOD")){
			read_ringmod(ringmods, ringmod_count++);
		}
		else if(! strcmp(modname, "REVERB")){
			read_reverb(reverbs, reverb_count++);
		}
	...
	initialize_globals(oscs, osc_count, noises, noise_count, sahs, 
		sah_count, vcfs, vcf_count, ringmods, ringmod_count, 
		reverbs, reverb_count);
	...
	for(i = 0; i < ringmod_count; i++){
		print_ringmod( ringmods[i]);
	  }
	for(i = 0; i < reverb_count; i++){
		print_reverb( reverbs[i]);
	}
...
void print_reverb(REVERB unit)
{
    printf("%s reverb %s, %s\n", unit.sig_out, unit.sig_in,
	   unit.rvbtime);
}
...
{
  scanf("%s %s %s", unit[count].sig_out,unit[count].sig_in1,
  unit[count].sig_in2);
}
...
{
  scanf("%s %s %s", unit[count].sig_out,unit[count].sig_in,
  unit[count].rvbtime);
}

void print_ringmod(RINGMOD rmod)
{
    printf("%s =  (%s) * (%s)\n", rmod.sig_out, rmod.sig_in1,
	   rmod.sig_in2);
}
