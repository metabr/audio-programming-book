...
void sahflt(int *vcount, char *invar, char *outvar);
void reverb(int *vcount, char *invar, char *outvar);
...
main(int argc, char **argv)
{
  ...
  if(rval < 0.333){
    ...
    reverb(&varcount, synvar, procvar1);
  } else {
    sahflt(&varcount, synvar, procvar1);
  }
rval = boundrand(0.0, 1.0);
  ...
  if(rval < 0.333){
    ringmod(&varcount, procvar1, procvar2);
  }
  else if(rval < 0.666) {
    reverb(&varcount, procvar1, procvar2);
  } else {
    sahflt(&varcount, procvar1, procvar2);
  }
...
void reverb(int *vcount, char *invar, char *outvar)
{
  ...
  rvbtime = boundrand(1.0, 4.0);

  /* NO INTERNAL VARIABLES ARE NEEDED SO WE JUST GRAB THE NEXT ONE FOR OUTPUT */

  sprintf(outvar, "ga%d", (*vcount)++);
  printf("REVERB %s (%s)*0.1 %f\n", outvar, invar,  rvbtime);
}
...
void sahflt(int *vcount, char *invar, char *outvar)
{
  ...
  speed = boundrand(1.0, 10.0);
  seed = boundrand(0.0, 1.0);
  min = boundrand(200.0, 400.0);
  max = boundrand(800.0, 1600.0);
  resonance = boundrand(0.7, 0.99);

  sprintf(v1, "ga%d", (*vcount)++);
  sprintf(outvar, "ga%d", (*vcount)++);

  printf("SAH %s %f %f %f %f\n", v1, speed, seed, min, max);
  printf("VCF %s %s %s %f\n", outvar, invar, v1, resonance);
}
